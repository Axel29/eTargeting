...
public class UserModel extends Model {
			...
			if (cookies != null) {
				for(int i=0; i<cookies.length; i++)
				{
					if ("user".equals(cookies[i].getName())) {
						...
						this.password  = userValues[1];
						this.email     = userValues[2];
						this.lastName  = userValues[3];
						this.firstName = userValues[4];
	...
	public void saveUserCookies(HttpServletResponse response) {
		try {
			...
			String[] userValues       = {Integer.toString(this.getUserId()), this.getPassword(), this.getEmail(), this.getLastName(), this.getFirstName()};
